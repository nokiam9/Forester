...
(function() {
    ...
    const settings = {
        ...
        post_base_url: 'http://127.0.0.1:4000/api/notices/',
    };
    ...
    (async function main() {
        ...
        for (let i = times; i > 0; i--) {
            ...
            if (nextPage(status, page_info) == 0) {
    ...
    function postOneNotice(notice, base_url){
        return new Promise((resolve, reject)=>{
            GM_xmlhttpRequest ({
                method:     "POST",
                url:        base_url + notice.nid, // TODO：base_url + notice.nid,
                ...
                try {
                    ...
                    for (let i = retry_times; i > 0; i--) {
                        ...
                        if (str.length <= min_length_of_content) {
                            ...
                            await sleep(1000);
                        } else resolve(str.replace(/(\n)/g, "").replace(/<script.*?>.*?<\/script>/ig, '')); // TODO:
                    }
